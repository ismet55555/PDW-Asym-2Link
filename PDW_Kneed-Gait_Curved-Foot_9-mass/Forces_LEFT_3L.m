function [Rx, Ry] = Forces_LEFT_3L(q, qd, qdd, parameters, FootData, r)
%This function computes the ground reaction forces for 3 Link LEFT stance
...
[psi, psi2, d_a, phi, theta_a, d_a2, phi2, theta_a2, height] ...
    = gContact('L', FootData, theta, q(1), q(2), r, 0, rLb, 0, LL, 0);
...
%Swing Knee Accelerations
a_n_knee = (ltR)*qd(2)^2;  
a_t_knee = (ltR)*qdd(2);

%ms2R at (c1R) from knee
a_n2 = (c1R)*qd(3)^2;  
a_t2 = (c1R)*qdd(3);
Rx = Rx + ms2R*(-a_n_hip *sin(beta_LL-phi) - a_t_hip *cos(beta_LL-phi) ...
               + a_n_knee*sin(q(2)-theta)  + a_t_knee*cos(q(2)-theta)  ... 
               +     a_n2*sin(q(3)-theta)  +     a_t2*cos(q(3)-theta));
Ry = Ry + ms2R*(-a_n_hip *cos(beta_LL-phi) - a_t_hip *sin(beta_LL-phi) ...        
               + a_n_knee*cos(q(2)-theta)  + a_t_knee*sin(q(2)-theta)  ...
               +     a_n2*cos(q(3)-theta)  +     a_t2*sin(q(3)-theta));

%ms1R at (c1R + b1R) from knee
a_n2 = (c1R + b1R)*qd(3)^2;  
a_t2 = (c1R + b1R)*qdd(3);
