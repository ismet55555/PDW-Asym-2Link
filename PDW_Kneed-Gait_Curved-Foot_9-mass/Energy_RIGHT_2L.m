...
function [E, KE, PE] = Energy_RIGHT_2L(q, qd, parameters, FootData, rR)
...
%mt2R at (lsR + c2R) (lower thigh)
dm_lsRc2R = sqrt((lsR + c2R)^2 + d_a^2 + 2*(lsR + c2R)*d_a*sin(theta_a));
beta_lsRc2R = -asin(((lsR + c2R)*sin(pi - theta_a + pi/2))/dm_lsRc2R);
psi_lsRc2R = -(beta_lsRc2R - phi + theta);
%mt1R at (lsR + c2R + b2R) (upper thigh)
dm_lsRc2Rb2R = sqrt((lsR + c2R + b2R)^2 + d_a^2 + 2*(lsR + c2R + b2R)*d_a*sin(theta_a));
beta_lsRc2Rb2R = -asin(((lsR + c2R + b2R)*sin(pi - theta_a + pi/2))/dm_lsRc2Rb2R);
...
%mh - Hip mass
v_mh   = [-dm_LR*q1d*cos(psi_LR),              -dm_LR*q1d*sin(psi_LR)              ];
...
v_ms2L = [(ltL+c1L)*q2d*cos(q2) - dm_LR*q1d*cos(psi_LR),...    %ms2L (upper shank)
    (ltL+c1L)*q2d*sin(q2) - dm_LR*q1d*sin(psi_LR) ];
v_ms1L = [(ltL+c1L+b1L)*q2d*cos(q2) - dm_LR*q1d*cos(psi_LR),...%ms1L (lower shank)
    (ltL+c1L+b1L)*q2d*sin(q2) - dm_LR*q1d*sin(psi_LR)];


%Rolling of stance foot:
v_roll = [-rR*q1d*cos(pi/2 - psi - theta),...
    rR*q1d*sin(pi/2 - psi - theta), 0];
v_roll = [0, 0, 0];


%Velocity Magnitudes:
v_mh_mag   = sqrt(   v_mh(1)^2   + v_mh(2)^2 );  %-  hip
v_ms1R_mag = sqrt( v_ms1R(1)^2 + v_ms1R(2)^2 );%\ RIGHT shank
v_ms2R_mag = sqrt( v_ms2R(1)^2 + v_ms2R(2)^2 );%/
v_mt2R_mag = sqrt( v_mt2R(1)^2 + v_mt2R(2)^2 );%\ RIGHT thigh
v_mt1R_mag = sqrt( v_mt1R(1)^2 + v_mt1R(2)^2 );%/

v_mt1L_mag = sqrt( v_mt1L(1)^2 + v_mt1L(2)^2 );%\ LEFT thigh
v_mt2L_mag = sqrt( v_mt2L(1)^2 + v_mt2L(2)^2 );%/
v_ms2L_mag = sqrt( v_ms2L(1)^2 + v_ms2L(2)^2 );%\ LEFT shank
...
KE = KE + 0.5*ms1R*(v_ms1R_mag^2);%\ RIGHT shank
KE = KE + 0.5*ms2R*(v_ms2R_mag^2);%/
KE = KE + 0.5*mt2R*(v_mt2R_mag^2);%\ RIGHT thigh
KE = KE + 0.5*mt1R*(v_mt1R_mag^2);%/

KE = KE + 0.5*ms1L*(v_ms1L_mag^2);%\ LEFT thigh
KE = KE + 0.5*ms2L*(v_ms2L_mag^2);%/
KE = KE + 0.5*mt2L*(v_mt2L_mag^2);%\ LEFT shank
...
PE = PE + ms1R*g* (dm_a1R*cos(psi_a1R));                      %\ RIGHT shank
PE = PE + ms2R*g* (dm_a1Rb1R*cos(psi_a1Rb1R));                %/
PE = PE + mt2R*g* (dm_lsRc2R*cos(psi_lsRc2R));                %\ RIGHT thigh
PE = PE + mt1R*g* (dm_lsRc2Rb2R*cos(psi_lsRc2Rb2R));          %/

PE = PE + mt1L*g* (dm_LR*cos(psi_LR) - (a2L)*cos(q2));        %\ LEFT thigh
PE = PE + mt2L*g* (dm_LR*cos(psi_LR) - (a2L+b2L)*cos(q2));    %/
PE = PE + ms2L*g* (dm_LR*cos(psi_LR) - (ltL+c1L)*cos(q2));    %\ LEFT shank
PE = PE + ms1L*g* (dm_LR*cos(psi_LR) - (ltL+c1L+b1L)*cos(q2));%/
