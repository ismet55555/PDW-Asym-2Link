...
%Plotting foot change rate vs work/step
figure
...
p_hit_1 = polyfit(hit_1(:,10), hit_1(:,14), 2);
p_hit_2 = polyfit(hit_2(:,10), hit_2(:,14), 2);
p_hit_3 = polyfit(hit_3(:,10), hit_3(:,14), 2);
pv_hit_1 = polyval(p_hit_1, x);
pv_hit_2 = polyval(p_hit_2, x);
pv_hit_3 = polyval(p_hit_3, x);

plot(hit_1(:,10), hit_1(:,14), '.k')
hold on
plot(hit_2(:,10), hit_2(:,14), '.b')
plot(hit_3(:,10), hit_3(:,14), '.r')
...
ylabel('Work/Step (Joules)','FontSize',12, 'FontName','Times New Roman')   
set(gca,'FontName','Times New Roman', 'FontSize',12);
legend('Ramp = 0.065', 'Ramp = 0.050', 'Ramp = 0.035')
title('PDW Walking Step Energy Expendature', 'FontSize',14, 'FontName','Times New Roman');
h_legend = legend();
...
%Regression Fits
x = linspace(-0.19, 0, length(hit_1(:,10)));
p_hit_1 = polyfit(hit_1(:,10), (hit_1(:,15)+hit_1(:,17))/2, 2);
p_hit_2 = polyfit(hit_2(:,10), (hit_2(:,15)+hit_2(:,17))/2, 2);
p_hit_3 = polyfit(hit_3(:,10), (hit_3(:,15)+hit_3(:,17))/2, 2);
pv_hit_1 = polyval(p_hit_1, x);
pv_hit_2 = polyval(p_hit_2, x);
pv_hit_3 = polyval(p_hit_3, x);

plot(hit_1(:,10), (hit_1(:,15)+hit_1(:,17))/2, '.k')
hold on
plot(hit_2(:,10), (hit_2(:,15)+hit_2(:,17))/2, '.b')
plot(hit_3(:,10), (hit_3(:,15)+hit_3(:,17))/2, '.r')
...
%Regression Fits
x = linspace(-0.19, 0, length(hit_1(:,10)));
p_hit_1 = polyfit(hit_1(:,10), (hit_1(:,16)+hit_1(:,18))/2, 2);
p_hit_2 = polyfit(hit_2(:,10), (hit_2(:,16)+hit_2(:,18))/2, 2);
p_hit_3 = polyfit(hit_3(:,10), (hit_3(:,16)+hit_3(:,18))/2, 2);
pv_hit_1 = polyval(p_hit_1, x);
pv_hit_2 = polyval(p_hit_2, x);
pv_hit_3 = polyval(p_hit_3, x);

plot(hit_1(:,10), (hit_1(:,16)+hit_1(:,18))/2, '.k')
hold on
plot(hit_2(:,10), (hit_2(:,16)+hit_2(:,18))/2, '.b')
plot(hit_3(:,10), (hit_3(:,16)+hit_3(:,18))/2, '.r')
