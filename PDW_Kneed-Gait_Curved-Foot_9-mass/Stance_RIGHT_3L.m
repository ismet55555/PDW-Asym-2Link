...
function [q, qdp, stop_condition, realtime, Energy, EnergyStat, Forces, ForcesStat, Angles, Ang_Vel] ...
                = Stance_RIGHT_3L(q,qd,parameters,params, steps, realtime, ...
...
time(1) = 0;
count = 2;
%Do loop until knee strike occurs where q2 and q3 are very close (within 0.005 or "is not a number")
while(abs(q(2,count-1) - q(3,count-1)) > 0.0075 & ~isnan(q))  

    %Getting Close to Knee Strike, increase resolution (slow down)
    if (abs(q(2,count-1) - q(3,count-1)) < 0.25) && (q(3,count-1) > 0);          
        dt = dt_dumdum;
    else
        dt = dt_dumdum + 0.002;  %Speed Up
        dt = dt_dumdum;
    end
    ...
    q3 = q(3, count - 1);       q3d = qd(3, count - 1);

    %Calling the right stance dynamics function
    [qdd(:,count)] = Dynamics_3L_RIGHT(q1, q1d, q2, q2d, q3, q3d, rLa, rLb, dL, rRa, rRb, dR, parameters, psi);
    ...
    if (force)
        Rx = 0;   Ry = 0;
        [Rx, Ry] = Forces_RIGHT_3L(q(:,end), qd(:,end), qdd(:,end), parameters, FootData, rR(count));
...
if(length(q) < 10) || (stop_condition == 1)
...
elseif(display)
    ...
    for loop = [1:n:length(q)-1, length(q)-1]
      ...
      height-(LR*cos(q(1,loop))+d_a*cos(pi-(pi/2 - q(1,loop))-theta_a)), 'color',[.2 .6 .8], 'markersize', 5, 'MarkerFaceColor', [.2 .6 .8])
        ...
        %=== Link 2 ===:     %RED - LEFT THIGH
        plot([0, ltL*sin(q(2,loop))],height+[0, -ltL*cos(q(2,loop))],'color',[.8 .6 .6],'LineWidth',6)
        %=== Link 3 ===:     %RED - LEFT SHANK
        plot([ ltL*sin(q(2,loop)), ltL*sin(q(2,loop))+lsL*sin(q(3,loop))],...
      height+[-ltL*cos(q(2,loop)),-ltL*cos(q(2,loop))-lsL*cos(q(3,loop))],'color',[.8 .6 .6],'LineWidth',6)
        
        %Plot link 2 base to ground contact:
        plot([ltL*sin(q(2,loop))+lsL*sin(q(3,loop)),  ltL*sin(q(2,loop))+lsL*sin(q(3,loop))+ d_a2*sin(pi-(pi/2 - q(2,loop))-theta_a2)],...
     height+[-ltL*cos(q(2,loop))-lsL*cos(q(3,loop)), -ltL*cos(q(2,loop))-lsL*cos(q(3,loop))- d_a2*cos(pi-(pi/2 - q(3,loop))-theta_a2)],...
            ':b','LineWidth',2,'color',[.8 .6 .6])

        %Plot curve top for swing       
        plot([ltL*sin(q(2,loop))+lsL*sin(q(3,loop))-(rRf(1)-dL)*sin(pi/2-q(3,loop)) ,        ltL*sin(q(2,loop))+lsL*sin(q(3,loop))+(dL+rLf(end))*sin(pi/2-q(3,loop))],...
      [height-ltL*cos(q(2,loop))-lsL*cos(q(3,loop))-(rRf(1)-dL)*cos(pi/2-q(3,loop)) , height-ltL*cos(q(2,loop))-lsL*cos(q(3,loop))+(dL+rLf(end))*cos(pi/2-q(3,loop))],...
            '-k','LineWidth',4)
        
        %LEFT foot contact:                 
        plot((LL*sin(q(3,loop)))+d_a2*cos(theta_a2 - q(3,loop)), ...
      height-(LL*cos(q(3,loop))+d_a2*sin(theta_a2 - q(3,loop))), ...
            'color',[.8 .6 .6], 'markersize', 5, 'MarkerFaceColor',[.2 .6 .8])

        
        %Plotting Foot Curves:
        %      Variable Structure: rim_dat = [rim_dat; d_a, theta_a, d_a2, theta_a2];
        for z = 3:length(qf)
            %Stance Foot Rim:           
            plot([(LR*sin(q(1,loop))) + rim_dat(z,1)*cos( rim_dat(z,2) - q(1,loop) ), (LR*sin(q(1,loop))) + rim_dat(z-1,1)*cos( rim_dat(z-1,2) - q(1,loop) )], ...
          [height-(LR*cos(q(1,loop)) + rim_dat(z,1)*sin( rim_dat(z,2) - q(1,loop) )), height-(LR*cos(q(1,loop)) + rim_dat(z-1,1)*sin( rim_dat(z-1,2) - q(1,loop)  ))],...
                 '-k','Linewidth',3)  
            %Swing Foot Rim:
             plot([ltL*sin(q(2,loop))+lsL*sin(q(3,loop))+rim_dat(z,3)*cos( rim_dat(z,4) - q(3,loop)   ),       ltL*sin(q(2,loop))+lsL*sin(q(3,loop))+rim_dat(z-1,3)*cos( rim_dat(z-1,4) - q(3,loop)  )], ...
           [height-ltL*cos(q(2,loop))-lsL*cos(q(3,loop))-rim_dat(z,3)*sin( rim_dat(z,4) - q(3,loop)   ),height-ltL*cos(q(2,loop))-lsL*cos(q(3,loop))-rim_dat(z-1,3)*sin( rim_dat(z-1,4) - q(3,loop)  )],...
                 '-k','Linewidth',3) 
        end
        ...
        %mt1L at (a2L)
        plot([(a2L)*sin(q(2,loop))],height+[-(a2L)*cos(q(2,loop))],'.k','MarkerSize',max([mt1L*mtmul 1]))
        %mt2L at (a2L+b2L)
        plot([(a2L+b2L)*sin(q(2,loop))],height+[-(a2L+b2L)*cos(q(2,loop))],'.k','MarkerSize',max([mt2L*mtmul 1]))
        
        %Link 3 masses:  (Swing Shank)
        %ms2L at (c1L)
        plot([(c1L)*sin(q(3,loop)) + (ltL)*sin(q(2,loop))],...
     height+[-(c1L)*cos(q(3,loop)) - (ltL)*cos(q(2,loop))],'.k','MarkerSize',max([ms2L*msmul 1]))
        %ms1L at (c1L+b1L)
        plot([(c1L+b1L)*sin(q(3,loop)) + (ltL)*sin(q(2,loop))],...
     height+[-(c1L+b1L)*cos(q(3,loop)) - (ltL)*cos(q(2,loop))],'.k','MarkerSize',max([ms1L*msmul 1]))
        ...
        if (save_walking_figure)  && (steps > 3) && (steps < 5)
            ...
            print('-djpeg','-r200',strcat(['C:\Users\ihandzic\Dropbox\REED LAB\Ismet\PDW\Ismet\PDW Simulation Model\PDW Curved Foot Model\PDW Curved Foot Three Link (9-mass)\Animation Video\LR3\' num2str(print_count) '.jpg']))
...
%                           RIGHT STANCE
%=====================================================================
q1 = q(1, end);       q1d = qd(1, end);
q2 = q(2, end);       q2d = qd(2, end);
                      q3d = qd(3, end);
...
[Qm2, Qp2] =        Knee_RIGHT(q1, q1d, q2, q2d, q3, q3d, FootData, rL, rR, parameters, psi);

qdm = [qd(1,end); qd(2,end); qd(3,end)];
qdp = Qp2^(-1)*Qm2*qdm;
