...
function [q, qdp, stop_condition, realtime, Energy, EnergyStat, Forces, ForcesStat, Angles, Ang_Vel] ...
            = Stance_LEFT_3L(q, qd, parameters,params,steps, realtime, ...
...
while(abs(q(2,count-1) - q(3,count-1)) > 0.0075 & ~isnan(q))
    ...
    [qdd(:,count)] = Dynamics_3L_LEFT(q1, q1d, q2, q2d, q3, q3d, rLa, rLb, dL, rRa, rRb, dR, parameters, psi);
    ...
    if(energy)
        [E_cont(count), KE(count), PE(count)] ...
             = Energy_LEFT_3L(q(:,end), qd(:,end), parameters, FootData, rL(count));
    end
    %========================================================
    ...
    if (force)
        Rx = 0;   Ry = 0;
        [Rx, Ry] = Forces_LEFT_3L(q(:,end), qd(:,end), qdd(:,end), parameters, FootData, rL(count));
...
elseif(display) 
    ...
    for loop = [1:n:length(q), length(q)]   
       ...
       [psi, psi2, d_a, phi, theta_a, d_a2, phi2, theta_a2, height] ...
        ...
        %=== Link 1 ===    %RED - LEFT
        plot([0, LL*sin(q(1,loop))], height+[0, -LL*cos(q(1,loop))],'color',[.8 .6 .6],'LineWidth',6) 
      ...
      height-(LL*cos(q(1,loop))+d_a*cos(pi-(pi/2 - q(1,loop))-theta_a)), 'color',[.8 .6 .6], 'markersize', 5,'MarkerFaceColor',[.8 .6 .6])
        ...
        %=== Link 2 ===:     %BLUE - RIGHT THIGH
        plot([0, ltR*sin(q(2,loop))],height+[0, -ltR*cos(q(2,loop))],'color',[.2 .6 .8],'LineWidth',6)
        %=== Link 3 ===:     %BLUE - RIGHT SHANK
        plot([ ltR*sin(q(2,loop)), ltR*sin(q(2,loop))+lsR*sin(q(3,loop))],...
      height+[-ltR*cos(q(2,loop)),-ltR*cos(q(2,loop))-lsR*cos(q(3,loop))],'color',[.2 .6 .8],'LineWidth',6)
        
        %Plot link 2 base to ground contact:
        plot([ltR*sin(q(2,loop))+lsR*sin(q(3,loop)),  ltR*sin(q(2,loop))+lsR*sin(q(3,loop))+ d_a2*sin(pi-(pi/2 - q(2,loop))-theta_a2)],...
     height+[-ltR*cos(q(2,loop))-lsR*cos(q(3,loop)), -ltR*cos(q(2,loop))-lsR*cos(q(3,loop))- d_a2*cos(pi-(pi/2 - q(3,loop))-theta_a2)],...
            ':b','LineWidth',2,'color',[.2 .6 .8])

        %Plot curve top for swing       
        plot([ltR*sin(q(2,loop))+lsR*sin(q(3,loop))-(rRf(1)-dR)*sin(pi/2-q(3,loop)) ,        ltR*sin(q(2,loop))+lsR*sin(q(3,loop))+(dR+rRf(end))*sin(pi/2-q(3,loop))],...
      [height-ltR*cos(q(2,loop))-lsR*cos(q(3,loop))-(rRf(1)-dR)*cos(pi/2-q(3,loop)) , height-ltR*cos(q(2,loop))-lsR*cos(q(3,loop))+(dR+rRf(end))*cos(pi/2-q(3,loop))],...
            '-k','LineWidth',4)
        
        %right foot contact:                 
        plot((LR*sin(q(3,loop))) + d_a2*cos(theta_a2 - q(3,loop)), ...
      height-(LR*cos(q(3,loop))+d_a2*sin(theta_a2 - q(3,loop))), ...
            'color',[.2 .6 .8], 'markersize', 5, 'MarkerFaceColor',[.2 .6 .8])
        ...
        for z = 3:length(qf)
            ...
            plot([(LL*sin(q(1,loop))) + rim_dat(z,1)*cos( rim_dat(z,2) - q(1,loop) ), (LL*sin(q(1,loop))) + rim_dat(z-1,1)*cos( rim_dat(z-1,2) - q(1,loop) )], ...
                 ...
                 '-k','Linewidth',3)  
            %Swing Foot Rim:
             plot([ltR*sin(q(2,loop))+lsR*sin(q(3,loop))+rim_dat(z,3)*cos( rim_dat(z,4) - q(3,loop)   ),       ltR*sin(q(2,loop))+lsR*sin(q(3,loop))+rim_dat(z-1,3)*cos( rim_dat(z-1,4) - q(3,loop)  )], ...
           [height-ltR*cos(q(2,loop))-lsR*cos(q(3,loop))-rim_dat(z,3)*sin( rim_dat(z,4) - q(3,loop)   ),height-ltR*cos(q(2,loop))-lsR*cos(q(3,loop))-rim_dat(z-1,3)*sin( rim_dat(z-1,4) - q(3,loop)  )],...
                 '-k','Linewidth',3) 
        end
        ...
        %ms2R at (c1R)
        plot([(c1R)*sin(q(3,loop)) + (ltR)*sin(q(2,loop))],...
     height+[-(c1R)*cos(q(3,loop)) - (ltR)*cos(q(2,loop))],'.k','MarkerSize',max([ms2R*msmul 1]))
        %ms1R at (c1R+b1R)
        plot([(c1R+b1R)*sin(q(3,loop)) + (ltR)*sin(q(2,loop))],...
     height+[-(c1R+b1R)*cos(q(3,loop)) - (ltR)*cos(q(2,loop))],'.k','MarkerSize',max([ms1R*msmul 1]))
        ...
        if (save_walking_figure)  && (steps > 3) && (steps < 5)
            ...
            print('-djpeg','-r200',strcat(['C:\Users\ihandzic\Dropbox\REED LAB\Ismet\PDW\Ismet\PDW Simulation Model\PDW Curved Foot Model\PDW Curved Foot Three Link (9-mass)\Animation Video\LL3\' num2str(print_count) '.jpg']))
...
[Qm2, Qp2]   =   Knee_LEFT(q1, q1d, q2, q2d, q3, q3d, FootData, rL, rR, parameters, psi);
