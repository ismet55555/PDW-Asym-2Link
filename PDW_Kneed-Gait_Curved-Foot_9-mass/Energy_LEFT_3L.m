...
function [E, KE, PE] = Energy_LEFT_3L(q, qd, parameters, FootData, rL)
...
%Right Shank (swing) Leg:
v_ms2R = [(c1R)*q3d*cos(q3)     + (ltR)*q2d*cos(q2) - dm_LL*q1d*cos(psi_LL), ... %ms2R (upper shank)
          (c1R)*q3d*sin(q3)     + (ltR)*q2d*sin(q2) - dm_LL*q1d*sin(psi_LL)];
v_ms1R = [(c1R+b1R)*q3d*cos(q3) + (ltR)*q2d*cos(q2) - dm_LL*q1d*cos(psi_LL), ... %ms1R (lower shank)
          (c1R+b1R)*q3d*sin(q3) + (ltR)*q2d*sin(q2) - dm_LL*q1d*sin(psi_LL)];
...
v_mt2L_mag = sqrt( v_mt2L(1)^2 + v_mt2L(2)^2 ); % / left (stance)
v_mt1L_mag = sqrt( v_mt1L(1)^2 + v_mt1L(2)^2 ); %/

v_mt1R_mag = sqrt( v_mt1R(1)^2 + v_mt1R(2)^2 );%\ right thigh(swing)
v_mt2R_mag = sqrt( v_mt2R(1)^2 + v_mt2R(2)^2 );%/
v_ms2R_mag = sqrt( v_ms2R(1)^2 + v_ms2R(2)^2 );%\ right shank(swing)
...
KE = KE + 0.5*ms1L*(v_ms1L_mag^2);%\
KE = KE + 0.5*ms2L*(v_ms2L_mag^2);% \
KE = KE + 0.5*mt2L*(v_mt2L_mag^2);% / left (stance)
KE = KE + 0.5*mt1L*(v_mt1L_mag^2);%/

KE = KE + 0.5*mt1R*(v_mt1R_mag^2);%\
KE = KE + 0.5*mt2R*(v_mt2R_mag^2);%/ right thigh (swing)
KE = KE + 0.5*ms2R*(v_ms2R_mag^2);%\
KE = KE + 0.5*ms1R*(v_ms1R_mag^2);%/ right shank (swing)
...
PE = PE + ms1L*g* (dm_a1L*cos(psi_a1L));                                  %\
PE = PE + ms2L*g* (dm_a1Lb1L*cos(psi_a1Lb1L));                            % \
PE = PE + mt2L*g* (dm_lsLc2L*cos(psi_lsLc2L));                            % / left (stance)
PE = PE + mt1L*g* (dm_lsLc2Lb2L*cos(psi_lsLc2Lb2L));                      %/

PE = PE + mt1R*g* (dm_LL*cos(psi_LL) - (a2R)    *cos(q2));                    %\ right thigh(stance)
PE = PE + mt2R*g* (dm_LL*cos(psi_LL) - (a2R+b2R)*cos(q2));                    %/
PE = PE + ms2R*g* (dm_LL*cos(psi_LL) - (ltR)    *cos(q2) - (c1R)    *cos(q3));%\ right thigh(stance)
PE = PE + ms1R*g* (dm_LL*cos(psi_LL) - (ltR)    *cos(q2) - (c1R+b1R)*cos(q3));%/
